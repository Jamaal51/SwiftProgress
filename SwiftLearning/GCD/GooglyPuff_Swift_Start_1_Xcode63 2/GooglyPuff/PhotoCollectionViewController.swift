...
private extension PhotoCollectionViewController {
  ...
  func showOrHideNavPrompt() {
    ...
    let popTime = dispatch_time(DISPATCH_TIME_NOW, Int64(delayInSeconds * Double(NSEC_PER_SEC))) //1 You declare the variable that specifies the amount of time to delay.
    
    dispatch_after(popTime, GlobalMainQueue){ //2 wait for the amount of time given in the delayInSeconds variable and then asynchronously add the closure to the main queue
      let count = PhotoManager.sharedManager.photos.count
      if count > 0 {
        self.navigationItem.prompt = nil
      } else {
        self.navigationItem.prompt = "Add photos with faces to Googleye them!"
    }
  }
}
