...
func BFS_print_nodes_with_distance(graph: Graph, startingVertex: Vertex){
    ...
    while (!queue.isEmpty){                 //stop when queue is empty
        ...
        for edge in graph.edges {                   //get adjacent vertices
            if edge.contains(v) {
                let neighborV = edge.getVertexPairFor(vertex: v)! //check that adjacent vertices have not been visited
                if neighborV.state == .Undiscovered{   //append undiscovered neighbors to the queue
                    queue.append(neighborV)
                    neighborV.currentDistance = v.currentDistance + 1
                    neighborV.state = .Discovered       //mark appended neighbors as discovered
                }
            }
        }
        print("\(v.name, v.currentDistance)")
        v.state = .Finished   //mark vertex as finished
    }
}

BFS_print_nodes_with_distance(graph, startingVertex: graph.vertices.first!)
