...
class PhotoDetailViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    dispatch_async(GlobalUserInitiatedQueue){ // 1 - You first move the work off of the main thread and onto a global queue. Asynchronous call oesn't lag view did load. ViewDidLoad finishes while face detection method finishes after
      
      let overlayImage = self.faceOverlayImageFromImage(self.image)
      
      dispatch_async(GlobalMainQueue){ //2 use this new image to update your UIImageView, you add a new closure to the main queue. UI updates are only the main queue
